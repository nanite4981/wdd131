...
function searchList(myList, query) {
  function searchCallback(item){
    if (item.name.toLowerCase().include(query.toLowerCase()) || item.description.toLowerCase().include(query.toLowerCase()) || item.tags.find((tag) => tag.toLowerCase().include(query.toLowerCase()))){
      return true;
    }
    else{
      return false;
  ...
  const filtered = myList.filter(searchCallback);
